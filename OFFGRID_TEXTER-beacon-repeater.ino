...
const char repeaton[] PROGMEM = {"  Repeater on.\n\n  Press B to stop."};
const char rcvmenu[] PROGMEM = {"Read MSG?\n A)Return\n B)Read\n C)Repeater"};
const char repeat[] PROGMEM = {"Repeat"};

const char *const string_table[] PROGMEM = {msgrecv,msgsent,counter,abcmode,ABCmode,nummode,sndmenu,beacon,beaconon,repeaton,rcvmenu,repeat};
...
void repeatmenu()
{
  ...
  if(!digitalRead(BUTTON_C))
      ...
      oled.display();
      oled.setCursor(0,15);
      oled.println(strcpy_P(buffer, (char *)pgm_read_word(&(string_table[9]))));
      oled.display();
      delay(6000);
      break;}
  }
...
if(sent == true&&readd == false){oled.setCursor(35,30);oled.println(strcpy_P(buffer, (char *)pgm_read_word(&(string_table[1]))));sent=false;}
if(beac == true&&readd == false&&muneco.length()>0){oled.setCursor(40,0);oled.println(strcpy_P(buffer, (char *)pgm_read_word(&(string_table[7]))));}
if(recv == true&&rpt == false&&readd == false){oled.setCursor(90,0);oled.println(strcpy_P(buffer, (char *)pgm_read_word(&(string_table[0]))));}
if(rpt == true&&readd == false){oled.setCursor(90,0);oled.println(strcpy_P(buffer, (char *)pgm_read_word(&(string_table[11]))));}
if(readd == true){oled.setCursor(0,0);oled.println(recvd);recvd="";readd=false;recv = false;}
oled.display();
}
...
void setup() {
  ...
  oled.begin(0x3C, true);  //can solder jumper to make into Ox3D i2c address
  oled.clearDisplay();
  oled.display();
  oled.setRotation(1);
...
if (!digitalRead(BUTTON_B))
  {
    ...
    oled.display();
    oled.setCursor(0,0);
    oled.println(strcpy_P(buffer, (char *)pgm_read_word(&(string_table[10]))));
    oled.display();
    repeatmenu();
  }
////////////////////////////////////////////
